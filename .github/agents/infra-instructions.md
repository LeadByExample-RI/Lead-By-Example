# Infra Agent — Deployment and Hosting

Purpose: Safe infra and deployment guidance specific to Vercel and the repo's deployment workflow.

Scope
- Files: `vercel.json`, `next.config.js`, `prisma/`, `package.json` scripts.
- Targets: Vercel builds, Prisma generation, environment variables, and production deploys.

Hard rules (must follow)
- Do not change deployment configuration (e.g., `vercel.json`, build commands) without explicit approval.
...
Quick links
- Build script: `package.json` (`vercel-build`, `postinstall`)
